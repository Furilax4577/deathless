...
- **Table** : `Assets/UI/Resources/DeathlessIcones.asset` (`Deathless.UI.IconesUI`) : identifiant (nom du SVG sans extension) → VectorImage. Reconstruite toute seule quand un SVG est ajouté, supprimé ou déplacé dans `Assets/UI/Icones/`, ou par **Deathless > UI > 6. Table des icônes (SVG)**. API : `IconesUI.Trouver(id)`, `IconesUI.Poser(element, id)` (fond de l'élément, masqué si l'icône manque), `IconesUI.Creer(id, classeUss)` ; classe USS `dl-icone` (image ajustée à la boîte, sans déformation). Constantes des icônes du HUD : `IconesUI.Mana`, `Rage`, `Furtif`, `Potion`, `Esquive`, `CoupCritique`, `MissileNyxessa` et `MissileNyxessaEteint` (compteur de missiles ; seules icônes du HUD en vert, famille `nyxessa` du générateur).
- **Liens action → icône** : dans les données, jamais dans le code des écrans. Actions des classes : `IActionClasse.Icone` et emblème `IClasseJouable.Embleme` (catalogue `ClassesJouables.Catalogue`, `Donnees/IClasses.cs`), lus par le HUD (`ClassesJouables.IconeAction(classe, action)`) et l'écran de choix de classe.
