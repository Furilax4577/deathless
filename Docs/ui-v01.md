...
| Options | `Options/Options.uxml`, `Options/Options.uss` | `EcranOptions` | Piste A « bandeau » (26/09/2026, voir ci-dessous) : panneau unique centré, onglets Jeu / Commandes / Audio en bandeau soudé au panneau (LB, RB ; Q/E). Jeu : pseudo, taille ×1 (80 %), ×2 (100 %), ×3 (135 %), et « Se relever » (marteler / maintenir, `OptionsJoueur.RelevageMaintenir`). Commandes : table en lecture seule (clavier et manette, la colonne manette suit la dernière manette), lignes focusables et défilantes. Audio : volumes principal, musique, effets spéciaux, interface (voir « Audio »). `Opaque` (26/09/2026) : masque l'écran de dessous (menu principal, ou le HUD sous la pause) tant qu'Options est ouvert. Pas de bouton Retour : l'invite Échap de la barre suffit. Réinitialiser (Y) : réglages de l'onglet affiché (taille ×2 et relevage par martelage, ou volumes par défaut). |
...
- Une **pile** : le sommet reçoit les actions ; on affiche l'écran opaque de base (HUD, menu, score) et le sommet par-dessus (pause, crédits). Un seul voile, une seule barre d'invites. Un écran superposé peut lui-même être `Opaque` (Options, 26/09/2026) : il masque alors tout ce qui est dessous (menu principal, ou pause) sans changer le comportement de ce qui est dessous une fois refermé.
...
### Options : piste A « bandeau » (26/09/2026)

Quentin a choisi la **piste A « bandeau »** parmi trois maquettes de l'écran Options (bac à sable `sandbox-ui`, `Assets/UI/Screens/OptionsMaquettes/`, scène `OptionsMaquettes.unity`, capture `maquette_options_A.png`), intégrée dans le vrai écran (`Assets/UI/Screens/Options/`, `Options.uxml` + `Options.uss` propre à l'écran) pour corriger le retour de Quentin sur l'ancienne mise en page (titre et onglets non alignés au panneau, contenu étroit collé sous le titre, menu principal visible en transparence derrière, bouton Retour flottant) :

- **Panneau unique centré** (`options-colonne`) : le titre et le panneau ont la même largeur et le même bord gauche, sur une grille de 8 px.
- **Onglets en bandeau soudé** en haut du panneau (`options-tabs`, `options-tab`), même largeur que le contenu : onglet actif (liseré or en bas), focus manette (anneau or), survol, et un fondu de 0,15 s (opacité) quand le contenu change d'onglet (`EcranOptions.AfficherPage`).
- **Jeu** : pseudo, taille ×1/×2/×3, et **« Se relever : marteler / maintenir »** (deux boutons pastille, `options-pill`, le choisi porte `dl-button--selected`), branchée sur `OptionsJoueur.RelevageMaintenir` (`Assets/Scripts/Jeu/OptionsJoueur.cs`) : accessibilité du relevé du Renversé (Wiki `statuts.md`), lue par `Heros.cs` (mode martelage ou maintien de Saut). Réinitialiser (Y) la remet à « Marteler » (défaut).
- **Commandes** : table en trois colonnes (action, clavier, manette) avec la pastille de touche (`InputPrompt`), dans un `ScrollView` interne (`options-scroll`) : le panneau ne déborde jamais.
- **Audio** : curseurs avec leur valeur affichée à droite (inchangé, classes renommées `options-volume*`).
- **Menu principal masqué derrière** : `EcranOptions.Opaque => true` (`NavigateurEcrans.Rafraichir` cache tout ce qui est sous le dernier écran opaque de la pile) — corrige l'ancien défaut où le menu principal restait visible en transparence, sans changer le comportement de la pause (`EcranPause` reste non opaque : le HUD continue d'apparaître dessous quand Options n'est pas ouvert par-dessus).
- **Bouton Retour retiré** : l'invite Échap de la barre d'invites (`UI/Cancel`, déjà présente) suffit.
- Tailles ×1/×2/×3 selon `ui-socle.md` : à ×3, la colonne « Jeu » (taille + relève) passe de deux colonnes à une seule empilée, jamais le texte courant réduit.
