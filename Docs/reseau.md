...
- `ReseauJeu.Assurer()` est appelé par `Partie.Awake` : l'objet `Reseau` naît au premier chargement du village et survit aux rechargements. Préfabs réseau enregistrés au démarrage : le salon, `PartieReseau`, les héros (registre `ClassesJeu`) et les squelettes du `DirecteurVagues` (sbire, guerrier, Golem, Nécromancien et les deux variantes de Morgrim, qui sont des préfabs distincts du Golem).
...
- **Morgrim, mini-boss nuit 10** (26/09/2026, wiki : `ennemis.md`) : deux versions (Massue, Martache), même squelette réseau que tout `Golem` (`EnnemiReseau`, `Statuts` — Étourdi, Ralenti). **L'hôte seul décide** : c'est lui qui tire au sort la version au crépuscule (`DirecteurVagues.Preparer`), choisit la compétence à chaque coup (distance, bouclier de Nyxessa, joueurs proches), applique les dégâts et pose les statuts (comme aujourd'hui pour le Golem). Position et déclencheur d'attaque partent du même chemin que les autres squelettes (`NetworkTransform`, `NetworkAnimator` en autorité serveur : le déclencheur `Attack` est répliqué, les autres postes voient donc le coup se jouer). **Effets au sol diffusés** (26/09/2026, chemin léger) : `EnnemiReseau.DiffuserEffetMorgrim(telegraphie, thème, rayon, durée, angle)` — un RPC serveur → clients (`[Rpc(SendTo.NotServer)]`), un octet de thème et quatre nombres, pas un message par gemme ; `MorgrimVariant.Telegraphier`/`Impact` l'appellent après avoir joué l'effet chez l'hôte, et `MorgrimVariant.RejouerEffetDistant` le rejoue chez chaque client devant sa propre marionnette (transform répliqué par `NetworkTransform`, léger décalage de latence accepté). Même limite que le Golem actuel pour l'onde de choc de `Golem.Frapper()` (`OndeDeChoc`/`DirtBurst`, non diffusée) : elle n'a pas été reprise ici, seuls les effets propres à Morgrim le sont.
  - **Onde de choc lente du Fracas** (Massue, 26/09/2026, wiki : `ennemis.md`, Morgrim massue) : un front qui s'étend lentement (`GameBalance.morgrimMassueFracasOnde*`), assez pour qu'on saute par-dessus ; non parable, non esquivable (`InfoDegats.parable = false`, ignore la garde, la parade et les frames d'invulnérabilité de la roulade). **Jugement côté client propriétaire** : avec la latence, l'hôte verrait les sauts des clients en retard, donc « au sol ou en l'air » se décide chez chaque client, sur l'onde qu'il voit. Départ et vitesse de l'onde sont diffusés par `EnnemiReseau.DiffuserOndeMorgrim(centre, vitesse, rayonMax, largeurBande)` — un second chemin léger, dédié, à côté de `DiffuserEffetMorgrim` (celui-ci reste purement visuel) : RPC serveur → clients (`OndeMorgrimRpc`) qui porte en plus l'heure de départ en temps réseau (`NetworkManager.ServerTime.TimeAsFloat`, comme `StatutsReseau`) ; chaque poste (l'hôte compris, pour son propre héros) fait tourner une `OndeChocLente` qui recalcule le même front que l'hôte (`front = min(vitesse × (tempsRéseau − départ), rayonMax)`) malgré la latence, et compare la position de son héros local au passage de ce front (bande de largeur `largeurBande`) ; en l'air (`Heros.AuSol` faux) : rien. Un client touché appelle `EnnemiReseau.SignalerOndeTouchee()` (RPC client → serveur) ; l'hôte **vérifie la vraisemblance** (front au bon endroit au bon instant à partir de sa propre horloge et de la position répliquée du héros, une touche par joueur et par onde) avant d'appliquer les dégâts et le statut Renversé (wiki : statuts.md) par le chemin habituel des coups (`Sante.Encaisser` sur la copie de l'hôte, relayée à son propriétaire si c'est une marionnette ; `Heros.Renverser`, relayé de même). Pour les marionnettes et les autres postes : visuel seulement (la même `OndeChocLente` tourne chez eux, mais ne juge jamais un héros qui n'est pas le leur).
- **Renversé** (knockdown, 26/09/2026, wiki : `statuts.md`) : `HerosReseau.Renverser()` (RPC hôte → propriétaire, `[Rpc(SendTo.Owner)]`, même chemin que `EncaisserRpc`) appelle `Heros.RenverserLocal()` chez le vrai propriétaire, qui déroule la séquence (chute, au sol, relevé) et pose le statut Renversé (wiki : statuts.md) comme les autres statuts posés sur soi (chute, garde brisée, ivresse : prédit aussitôt chez le propriétaire, confirmé par l'hôte). Le martelage de Saut qui accélère le relevé est **local au joueur renversé** : l'hôte n'en est pas informé coup par coup, il accepte simplement une fin de statut plus tôt que prévu, dans la limite du plafond (`GameBalance.renverseMartelementPlafond`, 50 % au plus).
...
- **Stock des missiles de Nyxessa** (26/09/2026, compteur du HUD) : l'hôte écrit le stock dans `PartieReseau.StockMissiles` (`NetworkVariable<int>`, envoyée seulement quand elle change) ; le palier passe déjà par `PalierMissiles`. La recharge du prochain missile voyage dans `PartieReseau.MissileRegeneration`, 5 fois par seconde (comme `TempsPhase`) ; chaque client complète seul entre deux envois et se recale dès qu'une nouvelle valeur arrive (`Partie.SuivreMissiles`).
  ...
  - Depuis la canalisation du bouclier (26/09/2026, `BouclierNyxessa.Absorber`), un coup encaissé peut faire **bondir** la recharge sans faire monter le stock : une simple extrapolation en `dt` ne suffisait plus à la suivre, d'où l'envoi périodique de sa valeur (avant le 26/09/2026, seul le stock voyageait et la recharge n'était qu'extrapolée).
...
- **Sorcier et bouclier** : l'hôte pilote ; les clients suivent (marionnette du sorcier, effet du bouclier levé, frappé, brisé, baissé). Réaction de coup du sorcier (26/09/2026, quand le bouclier encaisse un coup) : `PartieReseau.SorcierTouche` (RPC vers les clients, `Sorcier.ToucherDistant`), fréquence limitée côté hôte.
- Préfabs : `Deathless > Jeu > 8. Réseau` équipe aussi les 4 squelettes et crée `Assets/Jeu/Resources/Reseau/PartieReseau.prefab`.
...
## Parade parfaite (26/09/2026)

Jauge de parade et parade parfaite du paladin (wiki : `classe-paladin.md`, `interface.md`). Code : `Jeu/Heros/TelegraphieCoups.cs`, `Jeu/Classes/ParadeParfaite.cs`, `ClassePaladin`, `Squelette.AnnoncerCoup`, `HerosReseau` (`CoupAnnonceRpc`, `ParadeParfaiteRpc`).

Problème : la fenêtre parfaite dure 0,1 s. Un client ne peut pas attendre la réponse de l'hôte pour la juger : l'aller-retour vaut souvent autant.

- **Annonce du coup (hôte → propriétaire).** Quand un squelette commence la préparation d'un coup contre un héros (`Squelette.CommencerAttaque`), l'hôte envoie au propriétaire de ce héros un petit message : l'ennemi (`NetworkObjectId`) et la durée de la préparation (`HerosReseau.CoupAnnonceRpc`, `SendTo.Owner`). Un message par coup, rien pendant la préparation. Héros de l'hôte ou partie solo : annonce directe, sans réseau.
- **Impact prévu, dans l'horloge du client.** Le propriétaire note l'impact prévu : instant de réception + durée de la préparation (`TelegraphieCoups`). L'annonce, le coup (`EncaisserRpc`) et le geste du squelette (`NetworkAnimator`) partent tous de l'hôte et arrivent avec le même retard (la latence aller). L'impact prévu tombe donc là où le coup arrive vraiment chez lui, et là où il voit le coup porter. Il n'y a ni horloge à synchroniser ni correction de latence à estimer. Seule la gigue du réseau sépare les deux ; elle est couverte par une tolérance de 0,05 s (`paradeParfaiteGrace`).
- **Annulation sans message.** L'annonce s'efface si l'ennemi meurt, ou s'il est étourdi depuis le début de sa préparation (liste des statuts, déjà répliquée).
- **Jugement chez le propriétaire.** À l'appui sur LT, le client compare l'instant de l'appui à l'impact prévu. Dans la fenêtre parfaite, il lance tout de suite le coup de bouclier (geste, bond, éclat, son, tremblement), et le coup de cet attaquant sera paré à son arrivée. La parade normale se juge à l'arrivée du coup, comme avant (le coup est relayé au propriétaire, qui garde et pare). Elle est accordée si l'appui tombe dans la fenêtre, comptée depuis l'arrivée réelle du coup ou depuis l'impact prévu.
- **Validation par l'hôte, avec tolérance.** À l'instant du coup de bouclier, le client envoie `ParadeParfaiteRpc` (ennemi, direction, avance de l'appui). L'hôte l'accepte si trois conditions sont réunies :
  - ce squelette a bien annoncé un coup contre ce héros (`Squelette.CoupAnnonceSur`) ;
  - la demande arrive entre le début de la préparation et l'impact de l'hôte + le temps d'aller-retour du client (`GetCurrentRtt`, borné à 0,6 s) + 0,3 s (`paradeParfaiteToleranceReseau`) ;
  - le héros n'a pas fait d'autre demande depuis 0,3 s.
  Un refus est écrit au journal de la partie et n'a pas d'effet ; le client a seulement vu son geste.
- **Effets chez l'hôte.** L'hôte applique la repousse et l'étourdissement aux ennemis du cône, calculé depuis la position du héros qu'il voit (`ParadeParfaite.Appliquer` → `Squelette.Repousser`). Le statut Étourdi et le déplacement reviennent aux clients par les chemins habituels (`NetworkList` des statuts, `NetworkTransform`). L'étourdissement de la parade elle-même passe par `EtourdirRpc`, comme avant.
- **Vu des autres.** L'effet 8 du paladin (`E_Riposte`) rejoue l'éclat renforcé, le son et le geste du coup de bouclier sur la marionnette (`ClasseHeros.Diffuser`). Le geste est un `CrossFade` sur la couche haute, rejoué par l'effet parce qu'aucun déclencheur ne le porte.
- **Ce qui n'est pas protégé.** Un client peut mentir sur l'avance de son appui : la validation borne seulement la demande dans le temps et à un coup annoncé contre lui. C'est un jeu coopératif et l'enjeu est faible ; c'est le même niveau de confiance que les coups des clients.
- **Vérification** (26/09/2026, en solo dans l'éditeur, Village, `ScenariosParade`, guerrier devant et sbire à côté qui frappent ensemble) :
  - appui 0,19 s avant l'impact : parade des deux coups, tous deux étourdis ;
  - appui 0,04 s avant : parade parfaite, les deux ennemis repoussés (1,7 m et 1,2 m) et étourdis, aucun dégât ;
  - appui 0,43 s avant (hors de la fenêtre de 0,35 s) : coup bloqué ;
  - console sans erreur. Captures `Assets/Screenshots/parade_parfaite_jauge.png`, `_normale.png`, `_coup.png`.
  - Le chemin client (annonce, jugement, validation par l'hôte) compile mais n'a pas encore été essayé à deux postes.
...
- **Sacs des joueurs morts au donjon** (26/09/2026, décision de Quentin) : à la mort d'un joueur encore au donjon avec de l'or porté, l'hôte crée un sac à sa position (`DonjonJeu.CreerSac`) et le publie dans `PartieReseau.Sacs` (`NetworkList<SacReseau>` : id, position, montant, nom). Chaque poste suit cette liste (`DonjonJeu.SuivreSacsReseau`, comme `Pris` suit `ButinsPris`) et pose ou retire le visuel en conséquence (`SuivreSacsVisuels`) : modèle KayKit `sack.fbx` recalé sur le NavMesh, avec un scintillement de pièces (`SacDonjon`, purement visuel, non répliqué). Ramassage en marchant dessus, comme un tas d'or : un client demande le sac (`PartieReseau.DemanderSac`), l'hôte vérifie (sac encore là, distance) et l'accorde (`DonjonJeu.RamasserSac`), qui ajoute le montant à `EtatJoueur.orPorte` (répliqué comme le reste). Le joueur qui ramasse voit tout de suite « Tu ramasses le sac de X : N or » (message local optimiste, comme le dépôt à la caisse). **Économe** : un événement réseau à la création (`Sacs.Add`), un autre à la prise (`Sacs.RemoveAt`) ; à la fermeture du donjon (crépuscule), les sacs restants disparaissent en un seul événement (`Sacs.Clear`, `DonjonJeu.FermerSacs`), sans qu'il y ait à en envoyer un par sac.
- **Passages** : le propriétaire joue la téléportation (`PortalTransit`) et la diffuse avec `ClasseHeros.DiffuserTransit` (effets 203 et 204). La valeur libre `v` de l'effet porte le portail (0 aucun pour le rappel, 1 village, 2 retour du donjon) : les autres postes rejouent la dissolution vers ce portail et l'arrivée depuis lui, avec les anneaux (`DonjonJeu.TransitDistant`). Il saute de position avec `NetworkTransform.Teleport`, sans interpolation. Chez les autres, la marionnette est cachée entre le départ et l'arrivée.
  - **Animations d'entrée et de sortie** (26/09/2026, wiki : `portail.md`) : `DonjonJeu.Transit` attend la fin de l'effet de départ en gemmes (1,1 s) avant de téléporter, puis la fin de la reformation en gemmes avant de déclencher le clip d'arrivée (`Spawn_Air` au donjon, `Spawn_Ground` au village et au rappel de Nyxessa ; `Heros.DeclencherPortail`, durée 1,30 s, `PortailAnim`) ; le héros reste sans contrôle (`Heros.EnTransit`) jusqu'à la fin du clip. Le déclencheur passe par le `NetworkAnimator` du propriétaire (comme les emotes de la roue) : les autres postes voient le même clip sur la marionnette sans appel réseau supplémentaire ; `DonjonJeu.TransitDistant` ne fait que synchroniser les gemmes et la visibilité du corps sur ce même rythme. Sous-machine « Portail » commune aux 5 contrôleurs de classe (`Assets/Editor/Jeu/EmotesBuilder.cs`, `AjouterPortailArrivee`).
