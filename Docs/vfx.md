...
| **Nyxessa** | ombre : émeraude profonde `#062a17` · base : émeraude `#0b5226` · vif : vert Nyx `#178a36` · cœur : vert clair `#3fb552` · accent : éclat `#a4ec90` (26/09/2026, Quentin : vert plus profond ; **avant** : émeraude sombre `#145032`, émeraude `#1e5a32`, vert vif `#3fae5a`, vert clair `#9fe870`, éclat `#e8ffc8`, qui tiraient vers un menthe pâle presque blanc ; le cristal `RelicGem` ombre aussi ses facettes plus fort : 0,36 + 0,66 × lumière, éclaircissement de nuit 0,15 au lieu de 0,3) | gemme de la relique (`RelicMaterial.mat` = ombre, `RelicGem`), ceinture `RelicBelt`, portail `PortalVisual`, téléportation `PortalTransit`, missile crâne `SkullMissileVisual` (tiré par la relique), gerbes `GemBurst` (explosion / implosion / éclat) et traînée `GemTrail` ; **zone d'apparition des vagues** (`ZoneApparition`, exception décidée par l'utilisateur le 25/09/2026, voir sa fiche). Les dégradés de Relic (4 stops) sont repris sur base → vif → cœur → éclat (× 1,25 en HDR) : le vert est plus émeraude que le vert citron de Relic |
...
| **Soin** | ombre : or sombre `#b8903a` · base : or `#e8c872` · vif : or clair `#f4e2a8` · cœur : blanc chaud `#fff3d1` (26/09/2026, Quentin : blanc chaud et or, comme l'icône `paladin_soin` ; avant : les menthes, désormais dans SoinCroix) | aura de soin : paillettes `AuraGemmes` (or et or sombre), lumière de l'effet |
| **SoinCroix** (ajouté) | ombre : menthe profonde `#1b6a4c` · base : menthe sombre `#2e9e72` · vif : menthe `#4fcf9a` · cœur : menthe claire `#b8f5d8` | croix qui montent du soin (Lit : `AuraSoin_Menthe` = vif, `AuraSoin_MentheSombre` = base) : **elles restent vertes**, exception décidée par Quentin le 26/09/2026 |
...
Demande de Quentin : nuit plus sombre, tirée vers un bleu-violet (pas magenta), sans perdre la lisibilité, avec une légère brume au sol. Captures avant / après, même vue en partie : `Assets/Screenshots/nuit_avant.png`, `nuit_apres.png`.

- **Préréglage « nuit »** (`dusk`) de `Assets/VFX/_Ambiance/AmbianceVillage.asset`, appliqué par `DayCycle` et piloté par `CycleJourNuit` : lune (0,60 ; 0,52 ; 0,95) à 0,25 ; ambiante ciel (0,105 ; 0,078 ; 0,215), équateur (0,07 ; 0,05 ; 0,14), sol (0,02 ; 0,014 ; 0,042) ; brouillard (0,05 ; 0,03 ; 0,10) de 14 à 90 m ; ciel teinte (0,10 ; 0,055 ; 0,24), exposition 0,16. Le crépuscule et l'aube restent un fondu (SmoothStep de `DayCycle`) entre le jour et ce préréglage, avec la teinte chaude de `CycleJourNuit` au milieu.
- **`CycleJourNuit`** (scène Village et valeur par défaut du code) : `solCielNuit` (0,03 ; 0,018 ; 0,07), `intensiteNyxessa` 11 (au lieu de 9) pour que le vert ressorte sur le violet. Lanternes et fenêtres inchangées (chaudes).
...
- **Portail de retour du donjon** (26/09/2026) : le même prefab, instancié en jeu par `DonjonGenerateur` (`DonjonKit.portail`) sur le repère `PortailRetour` (centre à 1,85 m, disque face au nord du repère), avec `AlimenteParNyxessa` et `ReagirRelique` à faux, toujours ouvert. Sa boîte de collision passe sur la couche Ignore Raycast (hors NavMesh du donjon et hors caméra). Il remplace l'anneau de bronze (`Donjon_PortailRetour.mat`), gardé pour la génération dans l'éditeur.
...
- **Villageois sorcier** (banc, poste animé ; lore : le bouclier est invoqué par un villageois sorcier) : mannequin au style bâton (`Staff`), `Postes/BouclierRelique/Sorcier`, dans le cercle, face au centre. Il suit l'état du bouclier (`VfxBench.PosteSorcier`) :
  - **lever** (`IsCasting`) : `Ranged_Magic_Summon` (4,3 s) calé sur la durée de l'incantation (3 s), bras en haut quand le mur est complet ;
  - **tenue** (`IsUp`) : `Ranged_Magic_Spellcasting` (0,67 s) en boucle tant que le bouclier tient ;
  - **rupture** (vie à 0) : **`Death_A`** (`Rig_Medium_General`) : un choc à 0,1 s puis il est **projeté en arrière de 1,3 m** et au sol en 0,5 s ; il reste au sol 1,2 s puis se relève (`Death_A` joué à l'envers, 2,5 fois plus lentement, fondu vers le repos). Retenu parmi tous les clips Rig_Medium (General, Special, CombatMelee, CombatRanged, MovementAdvanced : `Hit_A/B`, `Death_A/B`, `Skeletons_Death`, `Melee_Block_Hit`, `Dodge_*` ; pas de `Knockback` ni de `Lie_*` dans les packs) parce que c'est le seul qui **projette** le personnage, à l'opposé de l'explosion du mur ; `Death_B` titube 1,3 s puis tombe en avant (épuisement, moins lisible), `Hit_B` n'est qu'un recul debout.
  - **Canalisation de Nyxessa** (amélioration, pas aux premiers rangs) : `VfxBench.canalisationNyxessa` (**faux par défaut**) : flux de gemmes Nyxessa **du cristal de la relique vers le bâton**, en arc (l'énergie coule vers le sorcier), tant qu'il incante. Sans l'amélioration, aucun lien.
  - Cycle du banc : `intervalleBouclier` = 18 s. Captures : `VfxBench_sorcier_invoque.png`, `VfxBench_sorcier_incante.png` (sans lien), `VfxBench_sorcier_canalise.png` (avec l'amélioration), `VfxBench_sorcier_rupture.png`.
...
- **En jeu** (25/09/2026) : `BouclierNyxessa` (objet de la scène Village sous Nyxessa, instance du prefab en enfant à +0,8 m) branche l'effet sur la partie : `Lever()` par le sorcier à la tombée de la nuit (encaissement du palier, `GameBalance.bouclierEncaissement`), absorbeur de la `Sante` de Nyxessa et du sorcier (chaque coup d'ennemi est pris par la paroi, le reste passe si le coup la brise), riposte `bouclierRenvoi` à l'attaquant, `Baisser()` à l'aube ; brisé → le sorcier meurt (`MortAllie`). Les squelettes frappent la paroi (rayon + 0,4 m). HUD : `IEtatPartie.Bouclier` / `BouclierMax`. Sorcier : `Sorcier` (prefab `Assets/Jeu/Prefabs/Sorcier.prefab`, modèle Mage, texture `mage_texture_alt_A` ; bâton `staff_B` du pack Fantasy Weapons Bits dans `handslot.r`, cristal passé au vert Nyxessa par une copie de l'atlas limitée aux pixels bleus, plus une légère émission sur ces pixels : `Assets/Jeu/Materiaux/Baton_Sorcier*` ; contrôleur `Sorcier_Jeu` : marche, invocation `Ranged_Magic_Summon` bâton levé calée sur l'incantation de 3 s, puis sort en boucle, mort `Death_A` vers l'arrière) ; menu `Deathless > Jeu > 9. Sorcier et bouclier`. Il incante dos à Nyxessa, face à l'extérieur, sur le sommet du plateau à 4,3 m du centre (à 1 m du bouclier), et pivote de ±20° au plus vers l'ennemi le plus proche devant lui ; mort, il tombe en arrière sur le sommet sans toucher le rocher. **Plateau agrandi** (Quentin, 25/09/2026) : marche du bas à r = 5,8 m (apothème 5,36 m), girons de 0,3 m, sommet de 5,2 m ; le bouclier (rayon 5,3 m, base à 0,25 m) repose sur le giron de la marche du bas (`VillageBuilder.RefairePlateau`, menu `Deathless > Village > Refaire le plateau de Nyxessa`, puis `6. Cuire le NavMesh`). Captures : `sorcier_jour.png`, `sorcier_nuit.png`, `sorcier_plateforme.png`, `sorcier_dos_nyx.png`, `sorcier_mort.png`, `sorcier_baton.png`, `bouclier_plein.png`, `bouclier_critique.png`, `bouclier_brise.png`, `bouclier_hud.png`.
...
- **Geste en jeu** (26/09/2026) : le même que celui du banc. Jambes en course, haut du corps en garde, coup de bouclier à l'arrivée, corps penché. Il est joué par l'Animator du Paladin : voir `Docs/styles-d-armes.md`, section « Charge bélier en jeu ».
...
### Mort et réapparition d'un allié (créé dans Deathless, 25/09/2026)
- **Prefab** : `Assets/VFX/MortAllie/MortAllie.prefab` (script `MortAllie`, matériau `PortalVoxel.mat`) ; une instance par scène (`MortAllie.Instance`).
- **Règle** : tout joueur ou PNJ du camp des joueurs qui meurt se dissout comme au départ d'une téléportation et **son énergie retourne à Nyxessa** ; à la réapparition, l'énergie part de Nyxessa et le personnage se recompose comme à l'arrivée d'une téléportation.
- **API pour le jeu** :
  - `MortAllie.Instance.Mourir(GameObject personnage, Nyxessa nyxessa, System.Action fin = null)` : les rendus du personnage sont coupés, dissolution `PortalTransit.Depart` (1,1 s) vers un point 0,3 m au-dessus du corps, puis flux `ChargeNyxessa.Lancer` de ce point au cristal (`Nyxessa.CentreCristal`), en arc (hauteur = distance × 0,2, bornée 1,2-5 m), durée = distance / 12 m/s (bornée 0,6-2,2 s) ; à l'arrivée, `Nyxessa.Reagir(PassageJoueur)` (onde sur la ceinture) puis `fin`. Jouer le clip de mort **avant** l'appel (le banc joue `Death_A`).
  - `MortAllie.Instance.Reapparaitre(GameObject personnage, Vector3 point, Nyxessa nyxessa, System.Action fin = null, System.Action recomposition = null)` : le personnage (caché) est placé en `point`, réaction de Nyxessa, flux du cristal vers le point, puis recomposition `PortalTransit.Arrive` (1 s) dans une capsule de 1,9 m ; les rendus reviennent à la fin, puis `fin`. `recomposition` : au début de la recomposition.
...
### Lueur légère la nuit (bloom, 26/09/2026)
- Volume URP global `VillageBlockout/Ambiance/Lueur_Nuit` (profil `Assets/VFX/_Ambiance/Lueur.asset` : Bloom seul, dispersion 0,55, plafond 8, qualité basse, sans salissure), piloté par `LueurNuit` d'après `CycleJourNuit.Nuit` : jour seuil 1,25 et intensité 0,12 (presque rien ne dépasse), nuit seuil 0,9 et intensité 0,7 : seules les vraies sources brillent (cristal de Nyxessa, anneaux du socle, portail, lanternes, fenêtres). Aucune allocation par image (profil copié une fois). Coupable : `LueurNuit.Active` (PlayerPrefs `Deathless.Lueur`, vrai par défaut), à brancher dans les options d'affichage. Menu `Deathless > Village > Lueur (bloom)`. Captures : `nyx_vert_avant_nuit.png`, `nyx_vert_apres_nuit.png`, `nyx_vert_apres_jour.png`.
