...
- **Barde, Bavaroise, Clochard (classes à venir, kits proposés)** : emblèmes `classe_<nom>_a` / `_b` (`classe_<nom>.svg` = variante retenue dans `NOUVELLES_CLASSES`, « a » par défaut) et une icône par action, jauge et passif : `barde_*`, `bavaroise_*`, `clochard_*`. Teintes relevées sur les modèles (palettes `BARDE`, `BAVAROISE`, `CLOCHARD` du script, à reporter en palettes du jeu avec leurs effets). Le script refuse toute teinte verdâtre (teinte entre 56° et 180°) hors Nyxessa : le gaz du clochard est un jaune-brun moutarde (teinte 37 à 42°).
- **Nyxessa (emblème du jeu)** : `Nyxessa/`, pour la page du wiki, le launcher et l'exe du jeu. Seules icônes en vert (palette Nyxessa), sans hexagone, avec un liseré à l'encre de l'interface pour les fonds clairs. Variantes `nyxessa_a` (gemme dans sa ceinture) et `nyxessa_b` (gemme, ceinture et rocher), chacune avec une version `_simple` (gemme plus large, liseré plus épais, moins de gemmes en orbite) rastérisée automatiquement jusqu'à 32 px (`NYXESSA_SEUIL_SIMPLE`). `nyxessa.svg`, `nyxessa_simple.svg`, `nyxessa_<taille>.png` (16 à 1024 px, fond transparent) et `nyxessa.ico` (16 à 256 px, entrées PNG) sont ceux de la variante retenue (`NYXESSA_CHOIX`, « a » par défaut). `raster.py` fait le rendu en Python pur (polygones suréchantillonnés, PNG par zlib, ICO à la main) ; `Launcher/make_icon.py` s'en sert et recopie `nyxessa.ico` dans `Launcher/Ressources/deathless.ico`.
- **Missile de Nyxessa (HUD)** : `Competences/nyxessa_missile.svg`, le crâne en gemmes vertes du jeu (`MissileCrane`) filant vers le haut à droite, courte traînée en diagonale ; glyphe seul, sans cadre, formes larges pour rester lisible à 30 px. Famille `nyxessa` : seule icône du HUD autorisée en vert. `nyxessa_missile_eteint.svg` en est la copie en ardoise (teintes `NYX_ETEINT`), posée dessous : le HUD découpe l'icône allumée du bas vers le haut pendant la recharge du prochain missile.
...
- **Statuts** (26/09/2026) : `Statuts/statut_<id>.svg` (brûlure, ralenti, étourdi, ivresse, provoqué), générés par `generer_statuts.py` (script frère, mêmes primitives et mêmes vérifications). Glyphe seul : le HUD dessine la case (liseré rouge d'affliction, or de bienfait) et la jauge de durée. Formes larges, lisibles à 24 px (au-dessus des ennemis, taille ×1). Couleurs : Feu (brûlure), Terre et Os (ralenti : escargot), Sacré (étourdi : étoiles d'or, comme l'indicateur du jeu), Terre, Fer, Os et Critique (ivresse : chope de bois), Rage (provoqué : veine de colère). `--png dossier` ajoute des aperçus à 128, 48 et 24 px.
...
| `nyxessa_missile` | Missile de Nyxessa (compteur du HUD, allumé) | aucun |
| `nyxessa_missile_eteint` | Missile de Nyxessa (même crâne en ardoise, sous l'allumé pendant la recharge) | aucun |
