...
- `Classes/classe_<classe>.svg` (5) : emblème dans un cadre hexagonal (les dalles du village), aux couleurs de la classe.
...
- **Couleurs** : lues à chaque génération dans `Assets/VFX/_Palettes/*.asset` (table de secours dans le script). Une icône prend le thème de son effet : Sacré pour le paladin, Feu pour le mage, Chasse (ocres seulement) pour le rôdeur, Ombre pour l'assassin, Rage pour le viking, Soin pour les soins, Critique pour le coup critique. Matières non magiques : bois = Terre, fer = accents Fer de Rage, plumes = ocres de Chasse. **Le vert Nyxessa est interdit** : le script s'arrête si une icône utilise une teinte de Nyxessa, les verts de Chasse ou l'accent Magie des Os. Exceptions : la jauge de mana reprend le bleu de la jauge du HUD (palette BouclierPlein), et l'esquive, sans thème, prend l'ivoire des Os.
