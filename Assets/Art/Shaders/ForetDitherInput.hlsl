// Copie de Packages/com.unity.render-pipelines.universal/Shaders/LitInput.hlsl (URP 17.3.0), à l'identique, avec un
// seul ajout : _Couverture dans le CBUFFER UnityPerMaterial (voir ForetDither.shader). Une copie complète est
// nécessaire parce que le SRP Batcher exige un seul CBUFFER nommé UnityPerMaterial par shader : on ne peut pas
// réutiliser LitInput.hlsl tel quel et ajouter une propriété par-dessus. Tout le reste (textures, fonctions
// d'échantillonnage, InitializeStandardLitSurfaceData) est inchangé : le rendu à _Couverture = 1 est celui d'URP Lit.
// À resynchroniser si le fichier d'origine change (changement de version d'URP).
#ifndef DEATHLESS_FORET_DITHER_INPUT_INCLUDED
#define DEATHLESS_FORET_DITHER_INPUT_INCLUDED
...
half _Surface;
half _Couverture;   // ajout Deathless (feuillage) : 1 = plein (rendu identique à URP Lit), vers 0 = de plus en plus clipé en damier
...
// _Couverture n'est pas exposé au DOTS instancing ci-dessous : le pack forêt de Deathless est posé comme des
// GameObjects classiques (VillageBuilder), jamais via Entities Graphics ; la surcharge se fait par
// MaterialPropertyBlock (Renderer.SetPropertyBlock), qui lit le CBUFFER ci-dessus directement.
...
#endif // DEATHLESS_FORET_DITHER_INPUT_INCLUDED
